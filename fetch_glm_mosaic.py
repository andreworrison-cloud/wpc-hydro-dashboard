...
The default execution path is intentionally operational and lean: it restores
five-minute cache slots, processes only missing slots, aggregates each satellite
once, and writes only the three dashboard-facing controlled mosaics plus their
metadata. The separate satellite references, ownership raster, raw-event
rasters, and radar-aware HTML remain available through ``--publish-debug-layers``
for manual diagnostics, but are not regenerated by the ten-minute production
job.
"""
...
TREND_MAP_LOW_SIGNAL_THRESHOLD = 10.0
TREND_MAP_MIN_RECENT_OR_PRIOR_VALUE = 3.0
TREND_MAP_CONTEXT_THRESHOLD = 6.0
TREND_MAP_NEIGHBORHOOD_RADIUS_CELLS = 2
TREND_MAP_ABSOLUTE_CHANGE_MIN = 2.0
TREND_MAP_RAPID_ABSOLUTE_CHANGE_MIN = 4.0
TREND_MAP_SMOOTHING_PASSES = 1
TREND_MAP_MIN_NEIGHBOR_SUPPORT = 3
TREND_MAP_MIN_COMPONENT_PIXELS = 2
...
def render_web_mercator(
    ...
    if np.issubdtype(source_values.dtype, np.floating):
        ...
        if np.issubdtype(source_values.dtype, np.unsignedinteger) and source_values.dtype.itemsize <= 2:
            source_uint16 = source_values.astype(np.uint16, copy=False)
        elif np.issubdtype(source_values.dtype, np.unsignedinteger):
            source_uint16 = np.minimum(
                source_values, np.array(np.iinfo(np.uint16).max, dtype=source_values.dtype)
            ).astype(np.uint16)
        else:
            source_uint16 = np.clip(
                source_values, 0, np.iinfo(np.uint16).max
...
def bin_counts(array: np.ndarray, bins: Sequence[int], labels: Sequence[str]) -> dict[str, int]:
    # Work only on nonzero/renderable cells. The operational fields are sparse,
    # so this avoids allocating a full-grid Boolean mask for every legend bin.
    values = array[array >= bins[0]]
    result: dict[str, int] = {}
    for index, lower in enumerate(bins):
        upper = bins[index + 1] if index + 1 < len(bins) else None
        mask = values >= lower
...
def _product_result_from_aggregate(
    satellite: str,
    window_minutes: int,
    end_time: datetime,
    desired: Sequence[datetime],
    ...
    config = SATELLITES[satellite]
    _, _, _, legend_id = legend_for_window(window_minutes)
    product_kind, short_display, units = product_text(window_minutes)
    display_label = f"{config['label']} — {short_display}"
    stem = f"glm_{satellite.lower()}_fed_{window_minutes}min"
    expected_slots = len(desired)
...
def _write_satellite_reference(
    result: ProductResult,
    aggregate: np.ndarray,
    desired: Sequence[datetime],
    selected: Sequence[datetime],
    ...
    bins, labels, colors, _ = legend_for_window(result.window_minutes)
    png_path = output_dir / result.leaflet_png
    metadata_path = output_dir / result.metadata_json
    bounds, rendered_shape, rendered_transform = render_web_mercator(
        aggregate, grid, png_path, args.maximum_render_dimension, bins, colors,
    )
    result.leaflet_bounds = bounds
    result.rendered_shape = rendered_shape
    metadata = {
        **asdict(result),
        "metadata_mode": "glm_dashboard_v1",
        "product_role": "satellite_reference_debug",
        "product": f"Incremental LCFA-derived GLM {result.product_kind}",
        "source_product": PRODUCT_PREFIX,
        ...
        "incremental_method": {
            ...
            "requested_slot_end_times_utc": [iso_z(value) for value in desired],
            "used_slot_end_times_utc": [iso_z(value) for value in selected],
            "missing_slot_end_times_utc": [iso_z(value) for value in desired if value not in selected],
            "cache_storage": "sparse nonzero grid indices and values in compressed NPZ files",
        },
        "methodology": (
            "Within each LCFA file, a quality-controlled flash contributes once to each 0.02-degree "
...
def prepare_satellite_products(
    satellite: str,
    end_time: datetime,
    available_times: set[datetime],
    cache_root: Path,
    ...
    grid: GridSpec,
    args: argparse.Namespace,
) -> tuple[dict[int, ProductResult], dict[int, np.ndarray]]:
    """Load each cached slot once and form all three rolling windows in one pass."""
    desired_by_window = {window: window_slot_times(end_time, window) for window in WINDOWS_MINUTES}
    selected_by_window = {
        window: [value for value in desired if value in available_times]
        for window, desired in desired_by_window.items()
    }
    for window, desired in desired_by_window.items():
        minimum_slots = math.ceil(len(desired) * args.minimum_window_completeness)
        if len(selected_by_window[window]) < minimum_slots:
            raise RuntimeError(
                f"{satellite} {window}-minute window has only "
                f"{len(selected_by_window[window])}/{len(desired)} cached slots"
            )
    ...
    metadata_by_time: dict[datetime, SlotMetadata] = {}
    desired_60 = desired_by_window[60]
    selected_30 = set(selected_by_window[30])
    selected_5 = set(selected_by_window[5])
    flats = {window: array.reshape(-1) for window, array in aggregates.items()}
    for slot_end in desired_60:
        if slot_end not in available_times:
            continue
        slot = load_sparse_slot(cache_root, satellite, slot_end)
        indices = slot["fed_indices"]
        values = slot["fed_values"].astype(np.uint32, copy=False)
        add_sparse(flats[60], indices, values)
        if slot_end in selected_30:
            add_sparse(flats[30], indices, values)
        if slot_end in selected_5:
            add_sparse(flats[5], indices, values)
        metadata_by_time[slot_end] = load_slot_metadata(cache_root, satellite, slot_end)

    results: dict[int, ProductResult] = {}
    for window in WINDOWS_MINUTES:
        selected = selected_by_window[window]
        slot_metadata = [metadata_by_time[value] for value in selected]
        result = _product_result_from_aggregate(
            satellite, window, end_time,
            desired_by_window[window], selected,
            aggregates[window], slot_metadata, grid,
        )
        if args.publish_debug_layers:
            result = _write_satellite_reference(
                result, aggregates[window], desired_by_window[window], selected,
                output_dir, grid, args,
            )
...
def build_source_ownership(grid: GridSpec) -> tuple[np.ndarray, dict[str, float]]:
    """Assign each column exclusively to the lower-view-angle satellite.

    Across this CONUS domain, cos(latitude) is positive and common to both
    satellite view-angle scores, so the ownership comparison depends only on
    longitude. Building a one-dimensional column mask avoids several full-grid
    float arrays on every production run.
    """
    lon = grid.west + (np.arange(grid.width, dtype=np.float64) + 0.5) * grid.resolution
    g18_sub = float(SATELLITES["G18"]["subsatellite_longitude"])
    g19_sub = float(SATELLITES["G19"]["subsatellite_longitude"])
    g18_score = np.cos(np.deg2rad(lon - g18_sub))
    g19_score = np.cos(np.deg2rad(lon - g19_sub))
    owner_columns = np.where(
        g18_score > g19_score,
        SATELLITES["G18"]["source_code"],
        SATELLITES["G19"]["source_code"],
    ).astype(np.uint8)
    owner = np.broadcast_to(owner_columns, (grid.height, grid.width)).copy()
    seam = (g18_sub + g19_sub) / 2.0
    return owner, {
        "g18_subsatellite_longitude": g18_sub,
        "g19_subsatellite_longitude": g19_sub,
...
def seam_statistics(
    ...
    lon = grid.west + (np.arange(grid.width, dtype=np.float64) + 0.5) * grid.resolution
    band_columns = np.abs(lon - seam_longitude) <= seam_band_degrees
    g18_band = g18[:, band_columns]
    g19_band = g19[:, band_columns]
    owner_band = owner[:, band_columns]
    both_nonzero = (g18_band > 0) & (g19_band > 0)
    either_nonzero = (g18_band > 0) | (g19_band > 0)
    selected = np.where(owner_band == 18, g18_band, g19_band)
    if np.any(both_nonzero):
        difference = g19_band[both_nonzero].astype(np.int64) - g18_band[both_nonzero].astype(np.int64)
    ...
    return {
        ...
        "band_grid_cells": int(grid.height * np.count_nonzero(band_columns)),
        ...
        "g18_nonzero_cells": int(np.count_nonzero(g18_band > 0)),
        "g19_nonzero_cells": int(np.count_nonzero(g19_band > 0)),
        "selected_nonzero_cells": int(np.count_nonzero(selected > 0)),
        "mean_g19_minus_g18_where_both_nonzero": mean_difference,
        "median_g19_minus_g18_where_both_nonzero": median_difference,
    }
...
def _box_filter_sum(array: np.ndarray, radius_cells: int) -> np.ndarray:
    ...
    return window_sum.astype(np.float32, copy=False)
...
def build_convective_trend_map_product(
    ...
    hour_sum = np.zeros((grid.height, grid.width), dtype=np.float32)
    recent_available = 0
    prior_available = 0
    hour_available = 0
    ...
    for slot_end in slot_times:
        ...
        if mosaic is None:
            continue
        values = mosaic.astype(np.float32, copy=False)
        hour_sum += values
        hour_available += 1
        if slot_end in prior_times:
            prior_sum += values
            prior_available += 1
        if slot_end in recent_times:
            recent_sum += values
            recent_available += 1

    classified = np.zeros((grid.height, grid.width), dtype=np.uint8)
    recent_smoothed = _box_filter_sum(recent_sum, TREND_MAP_NEIGHBORHOOD_RADIUS_CELLS)
    prior_smoothed = _box_filter_sum(prior_sum, TREND_MAP_NEIGHBORHOOD_RADIUS_CELLS)
    hour_smoothed = _box_filter_sum(hour_sum, TREND_MAP_NEIGHBORHOOD_RADIUS_CELLS)
    combined_smoothed = recent_smoothed + prior_smoothed
    difference = recent_smoothed - prior_smoothed
    denom = combined_smoothed.copy()
    change = np.zeros_like(recent_smoothed, dtype=np.float32)
    nonzero = denom > 0.0
    change[nonzero] = 200.0 * difference[nonzero] / denom[nonzero]

    if recent_available >= 2 and prior_available >= 2:
        context_mask = hour_smoothed >= TREND_MAP_CONTEXT_THRESHOLD
        significant = (
            context_mask
        ...
        rapidly_decreasing = significant & (change <= TREND_RAPID_DECREASE_THRESHOLD_PERCENT) & (abs_difference >= TREND_MAP_RAPID_ABSOLUTE_CHANGE_MIN)
        decreasing = significant & (change > TREND_RAPID_DECREASE_THRESHOLD_PERCENT) & (change <= TREND_DECREASE_THRESHOLD_PERCENT) & (abs_difference >= TREND_MAP_ABSOLUTE_CHANGE_MIN)
        rapidly_increasing = significant & (change >= TREND_RAPID_INCREASE_THRESHOLD_PERCENT) & (abs_difference >= TREND_MAP_RAPID_ABSOLUTE_CHANGE_MIN)
        increasing = significant & (change >= TREND_INCREASE_THRESHOLD_PERCENT) & (change < TREND_RAPID_INCREASE_THRESHOLD_PERCENT) & (abs_difference >= TREND_MAP_ABSOLUTE_CHANGE_MIN)
        steady = significant & ~(rapidly_decreasing | decreasing | increasing | rapidly_increasing)

        classified[rapidly_decreasing] = 2
        classified[decreasing] = 3
        classified[steady] = 4
        classified[increasing] = 5
        classified[rapidly_increasing] = 6
        for _ in range(max(0, int(TREND_MAP_SMOOTHING_PASSES))):
            classified = _majority_filter_classes(classified, TREND_MAP_BINS[1:])
        ...
        classified[(classified == 0) & context_mask] = 1
    ...
    metadata = {
        ...
        'comparison': 'Newest 15-minute neighborhood-summed contributions versus previous 15-minute neighborhood-summed contributions using symmetric percent change',
        'classification_thresholds_percent': {
            'rapidly_increasing': TREND_RAPID_INCREASE_THRESHOLD_PERCENT,
            'increasing': TREND_INCREASE_THRESHOLD_PERCENT,
            'decreasing': TREND_DECREASE_THRESHOLD_PERCENT,
        ...
        'low_signal_thresholds': {
            ...
            'recent_or_prior_period_value': TREND_MAP_MIN_RECENT_OR_PRIOR_VALUE,
            'past_hour_context_value': TREND_MAP_CONTEXT_THRESHOLD,
        },
        'smoothing': {
            'spatial_aggregation_neighborhood': f'{2 * TREND_MAP_NEIGHBORHOOD_RADIUS_CELLS + 1}x{2 * TREND_MAP_NEIGHBORHOOD_RADIUS_CELLS + 1} box sum',
            ...
            'absolute_change_minimum': float(TREND_MAP_ABSOLUTE_CHANGE_MIN),
            'rapid_absolute_change_minimum': float(TREND_MAP_RAPID_ABSOLUTE_CHANGE_MIN),
        },
        'input_slot_summary': {
            ...
            'hour_available_slots': hour_available,
        },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--publish-debug-layers", action=argparse.BooleanOptionalAction, default=False)
...
def main() -> int:
    ...
    print(f"Execution mode: {'full diagnostics' if args.publish_debug_layers else 'operational fast path'}")
    ...
    stage_timings["resolve_target"] = time.perf_counter() - stage_started
    ...
    try:
        ...
        for slot_end in required_times:
            ...
            if any(len(keys_by_satellite[s]) < minimum_files for s in SATELLITES):
                print(
                    ...
                    + ", ".join(f"{sat}={len(keys_by_satellite[sat])}/{EXPECTED_FILES_PER_SLOT}" for sat in SATELLITES),
                    file=sys.stderr,
                )
                continue
            build_slot_pair(client, slot_end, keys_by_satellite, cache_root, temporary_root, grid, args)
            slots_built += 1
        stage_timings["cache_update"] = time.perf_counter() - stage_started
        ...
        print(f"Incremental cache summary: hits={slots_hit}, built={slots_built}, retained={len(common_times)}")
        ...
        for satellite in ("G19", "G18"):
            satellite_results, satellite_arrays = prepare_satellite_products(
                satellite, target_end, common_set, cache_root, output_dir, grid, args,
            )
            for window in WINDOWS_MINUTES:
                result = satellite_results[window]
                results[(satellite, window)] = result
                arrays[(satellite, window)] = satellite_arrays[window]
                ordered_results.append(result)
        stage_timings["aggregate_windows"] = time.perf_counter() - stage_started
        ...
        ownership_metadata: dict = {
            "metadata_mode": "glm_dashboard_v1",
            "product_role": "source_ownership_debug",
            "methodology": "exclusive lower-view-angle source ownership",
            "satellite_geometry": geometry,
            "owned_cell_counts": {
                "G18": int(np.count_nonzero(owner == 18)),
                "G19": int(np.count_nonzero(owner == 19)),
        ...
        if args.publish_debug_layers:
            ...
            stage_timings["diagnostics"] = time.perf_counter() - stage_started
        ...
        stage_timings["total_before_manifest"] = time.perf_counter() - total_started
        ...
        manifest = {
            ...
            "execution_mode": "full_diagnostics" if args.publish_debug_layers else "operational_fast_path",
            ...
            "timing_seconds": {key: round(value, 3) for key, value in stage_timings.items()},
            "generated_time_utc": iso_z(utc_now()),
        }
        manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
        total_elapsed = time.perf_counter() - total_started
        print("Stage timings (seconds): " + ", ".join(
            f"{key}={value:.1f}" for key, value in stage_timings.items()
        ))
        print(f"Operational GLM generation completed in {total_elapsed:.1f} seconds through {iso_z(target_end)}")
        return 0
    finally:
        if args.keep_downloads:
            retained = output_dir / "downloaded_lcfa"
            if retained.exists():
