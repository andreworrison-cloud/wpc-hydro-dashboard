#!/usr/bin/env python3
"""Operational GOES GLM controlled CONUS mosaic generator.

This script retrieves public NOAA GOES-18 and GOES-19 GLM-L2-LCFA files,
resolves one synchronized end time, and derives LCFA-based 5-, 30-, and
60-minute lightning fields. It publishes three controlled CONUS mosaic PNGs
plus optional GOES-18/GOES-19 reference PNGs and the source-ownership mask.

The controlled mosaic uses exclusive lower-view-angle source ownership. Each
0.02-degree cell is assigned to one satellite only; values are copied without
summation, averaging, blending, or secondary-source gap filling.

Outputs are dashboard-ready transparent EPSG:3857 PNGs and compact JSON
metadata. This script writes to a staging directory; the GitHub workflow
validates the complete package before copying it into static/ and committing.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional UTC ending time shared by all windows. Blank selects "
            "the newest common GOES-18/GOES-19 end time."
        ),
    )
    ...
    parser.add_argument(
        "--publish-debug-layers",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Publish source ownership and separate GOES-18/GOES-19 reference layers.",
    )
...
def main() -> int:
    ...
    if args.maximum_latency_minutes <= 0:
        raise SystemExit("--maximum-latency-minutes must be positive")
    ...
    print(f"Operational GLM grid: {grid.height} rows × {grid.width} columns")
    ...
    if not args.analysis_time.strip():
        latency_minutes = (utc_now() - end_time).total_seconds() / 60.0
        if latency_minutes > args.maximum_latency_minutes:
            raise RuntimeError(
                f"Newest synchronized GLM end time is {latency_minutes:.1f} minutes old; "
                f"limit is {args.maximum_latency_minutes} minutes."
            )

    temporary_root = Path(tempfile.mkdtemp(prefix="glm_operational_"))
    try:
        ...
        print(
            "Controlled ownership: GOES-18 west / GOES-19 east of "
            f"approximately {seam_longitude:.2f}°; no blending or summation."
        )

        published_files: list[str] = []
        debug_products: list[dict] = []

        if args.publish_debug_layers:
            ...
            source_metadata = {
                ...
                "display_label": "GLM Debug — Mosaic Source Ownership",
                ...
                "grid": compact_grid_metadata(
                    grid, source_bounds, source_shape, source_transform
                ),
                "generated_time_utc": generated_time,
            }
            write_json(source_metadata_path, source_metadata)
            published_files.extend([source_png.name, source_metadata_path.name])
            debug_products.append(source_metadata)

            for satellite in ("G19", "G18"):
                satellite_label = SATELLITES[satellite]["label"]
                for window in WINDOWS_MINUTES:
                    bins, labels, colors, legend_id = legend_for_window(window)
                    product_kind, _, units = product_text(window)
                    start_time = end_time - timedelta(minutes=window)
                    stem = f"glm_{satellite.lower()}_fed_{window}min"
                    png_path = output_dir / f"{stem}.png"
                    metadata_path = output_dir / f"{stem}_metadata.json"
                    bounds, shape, transform = render_lightning_png(
                    ...
                    stats = aggregates[satellite].stats[window]
                    debug_label = (
                        f"GLM Debug — {satellite_label} — "
                        + ("Latest 5-Minute FED" if window == 5 else f"Rolling {window}-Minute Accumulation")
                    )
                    metadata = {
                        "metadata_mode": "glm_dashboard_v1",
                        ...
                        "window_minutes": window,
                        "product_kind": product_kind,
                        "display_label": debug_label,
                        "window_start_utc": iso_z(start_time),
                        "window_end_utc": iso_z(end_time),
                        "expected_files": stats.expected_files,
                        "listed_files": stats.listed_files,
                        "processed_files": stats.processed_files,
                        "completeness_fraction": stats.completeness_fraction,
                        "quality_controlled_flash_records": stats.quality_controlled_flash_records,
                        "events_mapped_to_good_flashes": stats.events_mapped_to_good_flashes,
                        "events_in_domain": stats.events_in_domain,
                        "flash_cell_contributions": stats.flash_cell_contributions,
                        "nonzero_grid_cells": stats.nonzero_grid_cells,
                        "maximum_value": stats.maximum_value,
                    ...
                    published_files.extend([png_path.name, metadata_path.name])
                    debug_products.append(metadata)

        mosaic_products: list[dict] = []
        ...
        for window in WINDOWS_MINUTES:
            ...
            metadata = {
                ...
                "default_opacity": 0.88,
                "grid": compact_grid_metadata(
                    grid, leaflet_bounds, rendered_shape, rendered_transform
                ),
                "rendering": {
                    "resampling": "nearest",
                    "bins": list(bins),
            ...
            write_json(metadata_path, metadata)
            published_files.extend([png_path.name, metadata_path.name])
            mosaic_products.append(metadata)
            summary_rows.append({
                "window_minutes": window,
                "window_start_utc": iso_z(start_time),
                "window_end_utc": iso_z(end_time),
                ...
                "mosaic_maximum": metadata["maximum_value"],
                "mosaic_nonzero_grid_cells": metadata["nonzero_grid_cells"],
                "mosaic_flash_cell_contributions": metadata["flash_cell_contributions"],
                **seam,
            })
        ...
        published_files.append(summary_path.name)

        manifest = {
            "metadata_mode": "glm_dashboard_v1",
            "product": "GOES GLM controlled CONUS mosaic operational package",
            "window_end_utc": iso_z(end_time),
            ...
            "mosaic_products": [
                {
                    key: item[key]
                    for key in (
                        "window_minutes", "display_label", "window_start_utc",
                        "window_end_utc", "maximum_value", "nonzero_grid_cells",
                        "flash_cell_contributions", "leaflet_png"
                    )
                }
                for item in mosaic_products
            ],
            "debug_layers_published": bool(args.publish_debug_layers),
            "published_files": sorted(published_files + ["glm_mosaic_manifest.json"]),
            "generated_time_utc": generated_time,
            "publishing_mode": "operational staging package",
        }
        manifest_path = output_dir / "glm_mosaic_manifest.json"
        write_json(manifest_path, manifest)

        print("\nOperational GLM staging outputs:")
